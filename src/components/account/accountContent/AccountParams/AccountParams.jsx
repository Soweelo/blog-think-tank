...
export default function AccountParams({
  ...
  const [option_delete_account, setOption_delete_account] = useState("");
  const [option_save, setOption_save] = useState("");
  const [option_update, setOption_update] = useState("");
  ...
  const deleteAccount = async () => {
    try {
      const response = await fetch(PF + "/api/members?token=" + user.session, {
        method: "DELETE",
      });
  ...
  useEffect(() => {
    ...
    setOption_update(getOptionByKey("06_update", allOptions));
    setOption_delete_account(getOptionByKey("06_delete_account", allOptions));
  }, [allOptions]);
  return (
    <div className="account-content__account-params-wrapper">
      ...
      <h2>
        ...
        <div
          ...
          dangerouslySetInnerHTML={{ __html: option_delete_account }}
        ></div>
      </h2>

      <div
        className={"account__form-message" + (message[0] ? " validate" : "")}
      >
        {message[1]}
      </div>
      {openConfirm && (
        ...
        <div className="account-content__info-line">
          ...
          {modifyPassword ? (
            <>
              <input
                ref={newPassword}
                className="account-content__value"
                placeholder="New password"
                disabled={isFetching}
              ></input>
              <button
                className="account__btn-submit btn"
            ...
            <>
              ...
              <button
                className="account-content__buttons-btn-edit btn"
                onClick={() => setModifyPassword(true)}
                dangerouslySetInnerHTML={{ __html: option_update }}
              ></button>
            </>
          )}
        </div>
